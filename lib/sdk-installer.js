...
function installAndroidSdk(apiLevel, target, arch, emulatorBuild, ndkVersion, cmakeVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!isOnMac) {
            yield exec.exec(`sh -c \\"chmod -R 777 ${process.env.ANDROID_HOME}"`);
        ...
        yield exec.exec(`sdkmanager --install 'build-tools;${BUILD_TOOLS_VERSION}' platform-tools 'platforms;android-${apiLevel}' > /dev/null`);
        if (emulatorBuild) {
            ...
            yield exec.exec(`sdkmanager --install emulator > /dev/null`);
        }
        console.log('Installing system images.');
        yield exec.exec(`sdkmanager --install 'system-images;android-${apiLevel};${target};${arch}' > /dev/null`);
        if (ndkVersion) {
            console.log(`Installing NDK ${ndkVersion}.`);
            yield exec.exec(`sdkmanager --install 'ndk;${ndkVersion}' > /dev/null`);
        }
        if (cmakeVersion) {
            console.log(`Installing CMake ${cmakeVersion}.`);
            yield exec.exec(`sdkmanager --install 'cmake;${cmakeVersion}' > /dev/null`);
        }
    });
}
