...
function launchEmulator(apiLevel, target, arch, profile, emulatorOptions, disableAnimations) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (profile.trim() !== '') {
            ...
            yield exec.exec(`echo no | avdmanager create avd --force -n test --abi '${target}/${arch}' --package 'system-images;android-${apiLevel};${target};${arch}'`);
        }
        // start emulator
        ...
        yield exec.exec(`${process.env.ANDROID_HOME}/emulator/emulator -avd test ${emulatorOptions} &`, [], {
            listeners: {
                stderr: (data) => {
